...
let FeaturedEvent = require('../models/FeaturedEvents');
...
router.get('/', authMiddleware, async (req, res) => {
  try {
    const EventDB = await FeaturedEvent.find();
    res.send(EventDB);
  } catch (err) {
...
router.get('/:id', authMiddleware,  async (req, res) => {
  try {
    const response = await FeaturedEvent.findById(req.params.id);
    if (!response) {
...
router.delete('/', authMiddleware, async (req, res) => {
  try {
    const Event = await FeaturedEvent.findOneAndRemove({ _id: req.body.id });
    if (!Event) {
      return res.status(404).send('Contact not found');
    }
...
router.put('/', authMiddleware, async (req, res) => {
  try {
    const Event = await FeaturedEvent.findById(req.body.id);
    if (!Event) {
